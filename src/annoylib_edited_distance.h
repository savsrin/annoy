...
struct Blosum{
  ...
  static inline bool side(const Node<S, T>* n, const T* y, int f, Random& random) {   
    T mgn = margin(n, y, f);
    bool final_side = margin(n, y, f) > n->avg_distance;
    if (__verbose) showUpdate("margin: %g, side: %d \n", mgn, final_side);
    if (__verbose) showUpdate("\n");
    return final_side; 
...
protected:
  ...
  S _make_tree(const vector<S >& indices, bool is_root) {
   ...
   if (_verbose) showUpdate("finished calling side on this node \n");
    ...
    if (failed) {
       if (_verbose) showUpdate("setting all dimensions of n->v to 0, num of indices to splt: %d \n", indices.size()); 
    }
