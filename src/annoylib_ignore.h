...
struct Blosum{
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n) {
    ...
    showUpdate("create split: ")

    for (int z = 0; z < f; z++) {
      n->v[z] = static_cast <int> ((p->v[z] + q->v[z])/2);
      showUpdate("printing children \n"); 
      showUpdate("%d (%d) (%d) (%d) \n", p->children[0], p->children[1] , q->children[0] , q->children[1]); 
      if((p->children[0] + q->children[0]) > (p->children[1] + q->children[1])){
        n->side = 0; 
      } else if((p->children[1] + q->children[1]) > (p->children[0] + q->children[0])){
        n->side = 1; 
      } else {
        n->side = random.index(2); 
      }
      showUpdate("n side: %d", n->side); 
      //showUpdate("%g (%d) ", n->v[z], static_cast <int> ((p->v[z] + q->v[z])/2));
    }
    showUpdate("\n");
    T min_dist = numeric_limits<T>::infinity(); 
    T max_dist = -(numeric_limits<T>::infinity());
    for (size_t j = 0; j < max_iterations; j++) {
      size_t rand_index = random.index(nodes.size()); 
      T dist = distance(nodes[rand_index], n, f);
      min_dist = std::min(min_dist, dist);
      max_dist = std::max(max_dist, dist); 
    }
    n->avg_distance = (min_dist + max_dist) / 2; 
  ...
  static inline T distance(const Node<S, T>* x, const Node<S, T>* y, int f) {
    ...
    T max_sim = 0; 
    for (int i = 0; i < f; i++) {
      max_sim += std:: max(scores62[(int) x->v[i]][(int) x->v[i]], scores62[(int) y->v[i]][(int) y->v[i]]);      
      blosum_sim += scores62[(int) x->v[i]][(int) y->v[i]];
    }
    //cout << max_dist - blosum_dist << "\n"
    return max_sim - blosum_sim;
    // nellore edit
    /*T blosum_sim = 0;
    T score1 = 0, score2 = 0; 
    for (int i = 0; i < f; i++) {
  ...
  static inline T margin(const Node<S, T>* n, const T* y, int f) {
   T blosum_sim = 0;
    T max_sim = 0; 
    for (int i = 0; i < f; i++) {
      max_sim += std:: max(scores62[(int) n->v[i]][(int) n->v[i]], scores62[(int) y[i]][(int) y[i]]);      
      blosum_sim += scores62[(int) n->v[i]][(int) y[i]];
    }
    return ((max_sim - blosum_sim) - n->avg_distance);  
    ...
    for (int i = 0; i < f; i++) {
      score1 = scores62[(int) n->v[i]][(int) n->v[i]]; 
      score2 = scores62[(int) y[i]][(int) y[i]];      
      blosum_sim += scores62[(int) n->v[i]][(int) y[i]];
    }
    //subtracting average distance from margin enables better splits in side
    return ((std::max(score1, score2)- blosum_sim) - n->avg_distance); */
  }

  template<typename S, typename T, typename Random>
  static inline bool side(const Node<S, T>* n, const T* y, int f, Random& random) {   
    T mgn = margin(n, y, f);
    bool final_side = n->side;
    showUpdate("margin: %g, side: %d \n", mgn, final_side);
    showUpdate("\n");
    return final_side; 

    //margin > average margin (T)
    //b4 children declare T avg_margin
  }
  template<typename T>
  static inline T normalized_distance(T distance) {
    return distance;
...
public:
  ...
  void build(int q) {
    showUpdate("building"); //TODO: remove
    if (_loaded) {
...
protected:
  ...
  S _make_tree(const vector<S >& indices, bool is_root) {
    ...
    showUpdate("finished calling side on this node");
    for (size_t i = 0; i < indices.size(); i++) {
      S j = indices[i];
      Node* n = _get(j);
      if (n) {
