...
struct Blosum{
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n) {
    ...
    //for(int j = )
      //random index n.size()
      //compute margin(n,random node) of random node
      //min (+infinity) and max margin (- infinity)
      //take average of min and max margin
      //n>avg distance = min + max / 2 
  ...
  static inline bool side(const Node<S, T>* n, const T* y, int f, Random& random) {   
    T mgn = margin(n, y, f);
    bool final_side = margin(n, y, f) > 0;
