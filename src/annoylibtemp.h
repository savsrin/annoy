...
struct Blosum{
  ...
  template<typename T> 
  static inline T blosum_distance(const T* u, const T* y, int f) {
    T blosum_sim = 0;
    T score1 = 0, score2 = 0; 
    for (int i = 0; i < f; i++) {
      score1 += scores62[(int) u->v[i]][(int) u->v[i]]; 
      score2 += scores62[(int) y->v[i]][(int) y->v[i]];   
      blosum_sim += scores62[(int) u->v[i]][(int) y->v[i]];
    }
