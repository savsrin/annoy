...
struct Blosum{
  ...
  template<typename T>
  static inline T pq_distance(T distance, T margin, int child_nr) {
    //pq from Minkowski, probably makes more sense
    if (child_nr == 0)
      margin = -margin;
    return std::min(distance, margin);
  }
  /*template<typename T>  //hamming pq distance, minkowkski is probably more relevant  
  static inline T pq_distance(T distance, T margin, int child_nr) {
    return distance - (margin != (unsigned int) child_nr);
  }*/ 
  ...
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n) {
    Node<S, T>* p = (Node<S, T>*)malloc(s); // TODO: avoid
    Node<S, T>* q = (Node<S, T>*)malloc(s); // TODO: avoid
    two_means<T, Random, Blosum, Node<S, T> >(nodes, f, random, false, p, q);
    for (int z = 0; z < f; z++)
      n->v[z] = p->v[z] - q->v[z];
    normalize(n->v, f);
    n->a = 0.0;
  ...
  template<typename T>
  static inline T pq_initial_value() {
    return numeric_limits<T>::max(); //where to define the max for blosum? 
  }
  ...
  static inline T distance(const Node<S, T>* x, const Node<S, T>* y, int f) {
    T dist = 0;
    for (int i = 0; i < f; i++) {
      dist += scores62[(int) x->v[i]][(int) y->v[i]];
    }
    return dist;
  }
  ...
  template<typename S, typename T>
  static inline T margin(const Node<S, T>* n, const T* y, int f) {
    //NEED TO implement blosum distance 
    T dist = 0;
    for (int i = 0; i < f; i++) {
      dist += scores62[(int) n->v[i]][(int) y[i]]; //VERY QUESTIONABLE,NEEDS TO BE FIXED 
    }
    return dist;

    
    //from euclidean: return n->a + dot(n->v, y, f);
  }

  /*template<typename S, typename T>  //margin method from hamming
  static inline bool margin(const Node<S, T>* n, const T* y, int f) {
    static const size_t n_bits = sizeof(T) * 8;
    T chunk = n->v[0] / n_bits;
    return (y[chunk] & (static_cast<T>(1) << (n_bits - 1 - (n->v[0] % n_bits)))) != 0;
 ...
 template<typename S, typename T, typename Random>
  static inline void create_split(const vector<Node<S, T>*>& nodes, int f, size_t s, Random& random, Node<S, T>* n) {
    size_t cur_size = 0;
    size_t i = 0;
    int dim = f; // was f*8*t for hamming
    for (; i < max_iterations; i++) {
      // choose random position to split at
      n->v[0] = random.index(dim); //storing index of dimension based on what you want to split
      cur_size = 0;
      for (typename vector<Node<S, T>*>::const_iterator it = nodes.begin(); it != nodes.end(); ++it) {
        if (margin(n, (*it)->v, f)) {
          cur_size++;
  ...
  template<typename S, typename T>
  static inline void init_node(Node<S, T>* n, int f) {
    //may need to implement omething here
  }
  static const char* name() {
    return "blosum";
  }
